...
stateDiagram-v2
    ...
    Invoke_TruffleHog --> Complete_With_Both: TruffleHog (verified / unverified / not_found)
...
Required for `trufflehog-only` and `hybrid` flows. Must be available on your system `PATH`.

```bash
# macOS (Homebrew)
brew install trufflehog

# Linux / macOS (Direct Binary Install)
...
| `trufflehog_result` | Enum | `verified`, `unverified`, `not_found`, `""` | Outcome of TruffleHog scan across the finding's line range. |
...
- **`trufflehog_result`**:
  ...
  - `unverified`: Secret signature matched, but live API verification failed or is unsupported.
  - `not_found`: Scanner ran but found no matching detection on the specified lines.
- **`llm_classification`**:
