...
This means all identity, grouping, and fetching logic derives from one parsed URL. The other columns (`repository`, `file path`, `lines`) are preserved verbatim in the output for downstream consumers but are not used for internal decisions.
